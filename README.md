...
VedaAI is a workflow-driven AI assessment generator for teachers. It allows teachers to configure assignment details, question types, marks, and reference materials, and uses Google's Gemini 2.5 Flash to automatically generate a complete, structured question paper that can be downloaded as a PDF.

## Features

- **2-Step Creation Flow**: Configure basic details, question types, and instructions.
- **AI Generation**: Powered by Gemini 2.5 Flash, returning structured JSON based on exact constraints.
...
3. Create a `.env` file (copy from `.env.example`):
   ...
   cp ../.env.example .env
   ```
   *Make sure to fill in your `GEMINI_API_KEY`, `MONGODB_URI`, and `REDIS_URL`.*
...
> **Note**: If you don't have Redis or a Gemini API Key, the backend gracefully degrades into **Mock Mode** and **Inline Generation Mode**, simulating AI responses so you can still test the UI!
...
- **Backend**: Ready for Railway/Render. Set `GEMINI_API_KEY`, `MONGODB_URI` (Atlas), and `REDIS_URL` (Upstash) in your hosting provider's secrets manager. Ensure you bind to the correct `PORT`.
